...
public class RoomController {
	...
	public ResponseEntity<?> checkAvailability(@PathVariable @DateTimeFormat(iso = ISO.DATE) LocalDate checkInDate){
		try {
			return ResponseEntity.status(HttpStatus.OK).body(roomService.findAvailableRoom(checkInDate));
		}
		catch (Exception e) {
			return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(new ApiResponce(e.getMessage()));
		}
